...
def routcounter (caveconnectionsdict):
    ...
    print("total number of possible routes with no doubling up of small caves: ", total_complete_routes)

routcounter (caveconnectionsdict)


# ---part 2 ------------------
# calculate possible routes with one case of doubling up a small cave

def routcounter2 (caveconnectionsdict):
    total_complete_routes2 = 0
    current_exploration_status = [{"lowercase_caves_visited": [], "did_I_visit_a_lowercase_cave_twice": False, "cave_I_am_in": "start"}]
    while len(current_exploration_status) != 0:
        status= current_exploration_status.pop(0)
        #print("lets check where we can go from:", status)
        ...
        for c in caveconnectionsdict[status["cave_I_am_in"]]:
            ...
            if c == "end":
                ...
                if c not in status["lowercase_caves_visited"]:
                    ...
                    current_exploration_status.append(new_status)
                    #print("yes, i can go to cave: ", c)
                elif c in status["lowercase_caves_visited"] and status["did_I_visit_a_lowercase_cave_twice"]== False:
                    new_status = copy.deepcopy(status)
                    new_status["cave_I_am_in"]= c
                    new_status["did_I_visit_a_lowercase_cave_twice"] = True
                    current_exploration_status.append(new_status)
            elif c.isupper():
                new_status = copy.deepcopy(status)
                new_status["cave_I_am_in"] = c
    ...
    #print( "final current pending exploration status: ", current_exploration_status)
    print("total number of routes: ", total_complete_routes2)

routcounter2 (caveconnectionsdict)
